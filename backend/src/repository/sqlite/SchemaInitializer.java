...
public class SchemaInitializer {
    ...
    public void initialize() {
        try (Connection connection = databaseManager.getConnection(); Statement statement = connection.createStatement()) {
            ...
            statement.executeUpdate("CREATE TABLE IF NOT EXISTS notifications (notification_id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY, source_event_id TEXT NOT NULL, recipient_user_id TEXT NOT NULL, recipient_role TEXT NOT NULL, event_type TEXT NOT NULL, message TEXT NOT NULL, occurred_at TEXT NOT NULL, is_read INTEGER NOT NULL)");
            ...
            statement.executeUpdate("INSERT INTO cancellation_policies(policy_id, cancellation_deadline_hours) VALUES ('default-cancellation', 24) ON CONFLICT (policy_id) DO NOTHING");
            statement.executeUpdate("INSERT INTO refund_policies(policy_id, refund_percent_before_deadline, refund_percent_after_deadline) VALUES ('default-refund', 100.0, 0.0) ON CONFLICT (policy_id) DO NOTHING");
            statement.executeUpdate("INSERT INTO pricing_policies(policy_id, allow_consultant_custom_price) VALUES ('default-pricing', 1) ON CONFLICT (policy_id) DO NOTHING");
            statement.executeUpdate("INSERT INTO notification_policies(policy_id, notify_on_booking_requested, notify_on_booking_accepted, notify_on_booking_rejected, notify_on_payment_processed, notify_on_booking_cancelled, notify_on_consultant_approval_decision) VALUES ('default-notification', 1, 1, 1, 1, 1, 1) ON CONFLICT (policy_id) DO NOTHING");
