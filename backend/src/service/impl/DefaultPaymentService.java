...
public class DefaultPaymentService implements PaymentService {
    ...
    public DefaultPaymentService(ClientRepository clientRepository, BookingRepository bookingRepository,
    ...
    @Override
    public SavedPaymentMethod addSavedPaymentMethod(String clientId, PaymentMethodType methodType, String displayLabel,
    ...
    public PaymentTransaction processPayment(String clientId, String bookingId, String savedMethodId) {
        ...
        try { Thread.sleep(ThreadLocalRandom.current().nextInt(2000, 4000));
        ...
        if (policyRepository.getNotificationPolicy().isNotifyOnPaymentProcessed()) {
            eventPublisher.publish(new PaymentProcessedEvent(eventPublisher.nextEventId(), LocalDateTime.now(), "Payment processed successfully for booking " + booking.getBookingId() + "."));
        }
        return transaction;
    }
    ...
    private void validatePaymentInput(PaymentMethodType methodType, String paymentDetails, String paymentDetailData) {
        ...
        switch (methodType) {
            ...
            case DEBIT_CARD: {
                ...
                if (parts.length != 2) {
                    throw new BusinessRuleViolationException(methodType.name() + " expiry and cvv must be in MM/YY|CVV format.");
                }
                ...
                if (!cvv.matches("\\d{3,4}")) {
                    throw new BusinessRuleViolationException(methodType.name() + " CVV must contain 3 or 4 digits.");
                }

                try {
                    YearMonth expiryMonth = YearMonth.parse(expiry, DateTimeFormatter.ofPattern("MM/yy"));
                    if (expiryMonth.isBefore(YearMonth.now())) {
                        throw new BusinessRuleViolationException(methodType.name() + " expiry must be current or future.");
                    }
            ...
            case PAYPAL:
                if (!details.matches("^[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\\.[A-Za-z]{2,}$")) {
